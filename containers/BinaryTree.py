...
These two classes are the building blocks for the BST, AVLTree, and Heap dat
a structures.
It is crucial to get these implemented correctly in order to be able to impl
ement the other data structures.
'''
...
class Node():
    ...
    Given a node t, you can visualize the node by running str(t) in the pyth
    on interpreter.
...
class BinaryTree():
    ...
    def print_tree(self, traversal_type):
        ...
        if traversal_type == 'preorder':
            ...
            message = 'Traversal type ' + str(traversal_type)
    ...
    def to_list(self, traversal_type):
        ...
        we can always print the returned results if we need to visualize the
        m.
        But by returning the results we can also do more computations on the
        results if needed.
        Many of the test cases for more complicated tree functions rely on t
        his to_list function,
    ...
    def __len__helper(node):
        ...
        child;
        if a right child exists, add the result of __len__helper on the righ
        t child;
    ...
    def _height(node):
        ...
        return 1 (for the current node) plus the max of the left and right _
        heights calculated above
        '''
        left = -1
        right = -1
        if not node:
